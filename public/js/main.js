...
for(var i = 0;i<imgLen;i++){
	//绝对路径来写
	arrImg.push('/public/images/img/'+i+'.jpg');
}
//2d找图全局变量
window.view = null;
...
function clearBugTab(obj,aLiWidth,aLiHeight,i){				//除bug，加载src;
	var yImg =  new Image();
	yImg.src = '/public/images/img/'+window.view+'.jpg';
	yImg.onload = function(){
...
function toJoin(){
	...
	if(window.mod3D){
		...
		for(var i = 0;i<aA.length;i++){
			clearInterval(aLi[i].dragTimer);
			clearInterval(aLi[i].timer);
			aA[i].style.backgroundImage = 'url(/public/images/img/'+ window.view +'.jpg)';
...
function picTab2D(num){
	...
	for(var i =0;i<aStrong.length;i++){			//strong记录当前
		...
		aStrong[i].style.backgroundImage = 'url(/public/images/img/'+ window.view +'.jpg)';
...
module3d.onclick = function(){
	...
	if(window.mod3D){				//2d参数
		window.mod3D = false;
		linkCSS.href = "/public/css/main.css" ;
		for(var i = 0;i<aLi.length;i++){
			clearInterval(aLi[i].timer);
			clearInterval(aLi[i].dragTimer)
		}
		...
		window.mod3D = true;
		linkCSS.href = "/public/css/module3d.css";
		for(var i = 0;i<aLi.length;i++){
			clearInterval(aLi[i].timer);
			clearInterval(aLi[i].dragTimer)
		}
